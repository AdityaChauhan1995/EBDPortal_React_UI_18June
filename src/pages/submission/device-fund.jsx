...
import { StaticBlock1, StaticBlock4 } from '../../components/common/dumb-component';
import { connect } from 'react-redux';
import { getFlexiFundDevices, fetchFundDeviceInfo,getRatePlanDeviceMapList,getRateplans,validateLineExistingGroup} from '../../redux/actions/configuration';
...
class DeviceFund extends Component {
	constructor(props) {
	  ...
	  this.state = {
		...
		validatedDeviceList:props.validatedDeviceList,
		totalFundAmountInitial:'',
	...
	componentDidMount(){
		...
		this.setState({totalFundAmount:tempTotalFundAmount,totalFundAmountInitial:tempTotalFundAmount, totalAdjAmount: adjAmountCurrent });
	...
	componentWillReceiveProps(nextProps){
		...
		if(this.props.FETCH_FLEXI_FUND_DEVICES_STATUS !== 'SUCCESS' &&
			nextProps.FETCH_FLEXI_FUND_DEVICES_STATUS === 'SUCCESS' ){
		...
		if(this.props.GET_RATEPLAN_DEVICE_MAP_LIST_STATUS !== 'SUCCESS' &&
			nextProps.GET_RATEPLAN_DEVICE_MAP_LIST_STATUS === 'SUCCESS'){
				this.setState({showDimmer:false,
								ratePlanDeviceMapList:nextProps.ratePlanDeviceMapList});
		...
		if(this.props.VALIDATE_LINE_EXISTING_GROUP_STATUS !== 'SUCCESS' &&
			...
			if(validatedDeviceList !== undefined && validatedDeviceList !== null && validatedDeviceList.length >0){
				...
				if(!validateDevice){
					...
					this.props.setDeviceDetails(tempAddedDevices, totalPrice, totalQuantity, deviceFundUsed, deviceFundLeft,
													amountPayable, 0, previousFundAmount,previousFundAmountUsed, previousFundAmountLeft, totalAdjAmount);
				}else{
					let totalPrice=0;
					let totalQuantity=0;
	...
	next=()=>{
		...
		let { addedDevices, totalFundAmount, amountPayable, tax , totalPrice, totalQuantity, deviceFundUsed, deviceFundLeft }=this.state;
		let fundDeviceList=[];
		let fundSummary='';
		...
		if(msisdnList === null || msisdnList.length === 0){
			...
			if((addedDevices !== undefined && addedDevices !== null && addedDevices.length !==0) && !this.state.validateDevice){
				this.setState({validateStatus:'FAILURE',message:'Please click VALIDATE DEVICE before proceeding'});
			}
			else{
				this.props.setDeviceFundInfo(fundDeviceList,fundSummary,msisdnList, addedDevices, totalQuantity, deviceFundUsed, deviceFundLeft, amountPayable, tax,0);
	...
	addDevice=()=>{
		...
		let { orderCategory, previousFundAmount, previousFundAmountUsed, checkEligibilityforFundDevice }=this.props;
		if(previousFundAmount!==undefined && orderCategory==='Existing Group'){
			if(!checkEligibilityforFundDevice){
				this.setState({validateStatus:'FAILURE',message:'Seems like your previous devices are still not processed, first previous devices need to be delievered then only new Fund Device can be taken.'});
				return;
			}
		}
		...
		if(totalFundAmount!=='' && parseFloat(totalFundAmount)<=0 && orderCategory!=='Existing Group'){
			...
			if(addedDevices != undefined && addedDevices != null && addedDevices.length>0){
				...
				// 	value:{...this.props.fundDeviceInfo,quantity:1,eligibleFlexiFundContract:null,adjAmount:null}
				// }];

				var tempAddedDevice=[...addedDevices,{
						key:maxKeyValue+1,
						value:{...this.props.fundDeviceInfo,quantity:1, eligibleFlexiFundContract:null, contractMsisdn:null, adjAmount:null}
					}];
				maxKeyValue++;
				...
				console.log('add device',tempAddedDevice);
	...
	removeRow(row){
		let { addedDevices, totalFundAmount, totalAdjAmount }= this.state;
		if(addedDevices===undefined){
			addedDevices=[];
		}
	  	...
	  	if(orderCategory!==undefined && orderCategory==='Existing Group' ){
	  		if(previousFundAmount!==undefined && parseFloat(previousFundAmount) >=0){
	  			...
	  			if(parseFloat(previousFundAmountLeft)<=0){
	  				...
	  				previousFundAmountLeft=0;
	  				amountPayable=totalPrice-previousFundAmount;
	  				let fundAmountLeft=totalFundAmount-amountPayable;
	  				if(parseFloat(fundAmountLeft)<=0){
	  					fundAmountUsed=totalFundAmount;
	  					fundAmountLeft=0;
	  					amountPayable=totalPrice-(totalFundAmount+previousFundAmount);
	...
	validateDevicesNewGroup = () =>{
		let{ratePlanDeviceMapList,addedDevices, totalAdjAmount}=this.state;
		let{deviceFunds} = this.props;
									...
									for(var k=0;k<tempDeviceFundsWithoutMNP.length;k++){
									  if(tempDeviceFundsWithoutMNP[k] != undefined && tempDeviceFundsWithoutMNP[k] != null){
										if(tempRatePlan[j] === tempDeviceFundsWithoutMNP[k].ratePlanId
											...
											// tempDeviceFundsWithoutMNP[k].msisdn;
										...
										for(var k=0;k<tempDeviceFundsWithMNP.length;k++){
										  if(tempDeviceFundsWithMNP[k] != undefined && tempDeviceFundsWithMNP[k] != null){
											if(tempRatePlan[j] === tempDeviceFundsWithMNP[k].ratePlanId
												...
												//ratePlanDeviceMapList[i].maxDeviceAllowed[j] = tempMaxDeviceAllowed[j]- 1;
												// currentDevice.value.eligibleFlexiFundContract = tempDeviceFundsWithMNP[k].msisdn;
