...
import { Grid, Input, Button, Checkbox, Segment, Container, Icon, Form, Dropdown, Message, Confirm, Label } from "semantic-ui-react";
import Navigation from '../../components/header/navigation';
import { PrimaryButton, SecondaryButton } from '../../components/common/buttons';
import { StaticBlock1 } from '../../components/common/dumb-component';
...
import { setProductOrderInfo, setLineDetails, getFundAmount, checkEligibilityForFundDevice, getRatePlanData, setRatePlanBlank } from '../../redux/actions/order';
...
const DeviceInfo = ({ regType, simCardNo, simTypes, simType, mobileNumber, deviceContract, zerolutionDevices, phoneModel, handleChange, handleChangeCheckBox, contractCheck, fetchRatePlanDetails ,orderCategory, ratePlans, ratePlan}) => {
	return (
		<React.Fragment>
...
class ProductOrder extends Component {
	constructor(props) {
		...
		this.state = {
			...
			nextUrlAddVas: url.substring(0, url.lastIndexOf("/")) + "/order-submission",
			searchStatus: '',
			todos: props.todos,
			currentPage: 1,
			todosPerPage:10,
			...
			pkgList:[],
			contractRenewalRatePlans:[],
			contractRenewalRatePlan:'',
			contractRenewalInfo:[],
			contractInfoDataCR:[]
		};
	...
	componentDidMount() {
		...
		if(this.state.deviceFulfillment == '')
		{
	...
	componentWillReceiveProps(nextProps) {
		if (this.props.FETCH_REG_TYPES_STATUS !== 'SUCCESS' &&
			...
			if(orderCategory!==undefined && orderCategory!==null && regType!==undefined && regType!==null && 
				...
				this.setState({mobileNoValidSubscriber:true, msisdnContracts:nextProps.msisdnContracts, showDimmer: false, isContractCheckDone:true,isActiveKenanContractPresent:isActiveKenanContract,contractInfoDataCR:contractInfoData});
			...
			let { todos, regType, mobileNumber, ratePlan, currentPage, simCardNo, simType, donorAccountNo, lineDeviceInfo, 
				 donorType, flexiFundContractTaken, ratePlanId, regTypeId, vasList, iddList, byodContractTaken, crpLineCount, addOnVal ,isActiveKenanContractPresent,contractInfoDataCR} = this.state;
				 console.log(vasList);
...
            } 
		...
		}else if (isChangedToRejected(this.props.FETCH_RATEPLANS_STATUS, nextProps.FETCH_RATEPLANS_STATUS)) {
			this.setState({ status:'FAILURE', message:'Error in fetching rateplans', showDimmer: false	 });
		}
	...
	next = () => {
		...
		if(todos===null || todos===undefined || todos.length===0){
			if(orderCategory==='New Group'){
				this.setState({ status: 'FAILURE', message: 'You must select mininum of 5 lines apart from offers with 48 Plan to proceed & max '+this.props.tempMaxLineCount+' lines can be selected' });
				return;
			}
			else{
		...
		if(orderCategory!==undefined && orderCategory==='New Group'){
			if(todos.length-tempDataFor48.length<5 || todos.length>this.props.tempMaxLineCount){
				this.setState({ status: 'FAILURE', message: 'You must select mininum of 5 lines apart from offers with 48 Plan to proceed & max '+this.props.tempMaxLineCount+' lines can be selected' });
				return;
			}
		}
			...
			if(todos.length!==0 && (todos.length-tempDataFor48.length-tempCrpLineCount+lineCount)<5 && counter===0){
				this.setState({ status: 'FAILURE', message: 'You must select mininum of 5 lines apart from offers with 48 Plan to proceed & max '+this.props.tempMaxLineCount+' lines can be selected. Close this message if you would like to Proceed.', counter:1 });
				return;
			}
			if(((todos.length+lineCount-tempCrpLineCount)>this.props.tempMaxLineCount)){
				this.setState({ status: 'FAILURE', message: 'Max of '+this.props.tempMaxLineCount+' lines can be selected' });
				return;
			}
			if(todos.length===0){
		...
		if (status === 'SUCCESS') {
			...
			// this.props.checkingBccValidation(brn,totalRequestingLine,requestVasIds);
	...
	addLine = () => {
		let { todos, regType, mobileNumber, ratePlan,
			...
			flexiFundContractTaken, ratePlanId, mobileNoValidSubscriber, isContractCheckDone, byodContractTaken, crpLineCount, isaddMemberCheckValid, ratePlanPkgId } = this.state;
		let { orderCategory }=this.props;
		...
		if(regType !== 'Change Subscription' && regType !== 'Contract Renewal' &&  this.props.orderCategory!=='Existing Group- Add VAS' ){
	...
	fetchRatePlanDetails=()=>{
		let { mobileNumber, ratePlan }=this.state;
	...
	select(checkedVAS, checkedIDDCountries, isVasRUleCheckValid) {
		if(isVasRUleCheckValid==='Valid'){
			...
			if(isVasRUleCheckValid==='noActionPerformed'){
				...
				this.setState(({'isVasRUleCheckValid':'Invalid'}));
	...
	render() {
		...
		let { regTypes, ratePlans, simTypes, zerolutionDevices, donorTypes, brn, orderCategory, selectedVSN, portalCustInfo, addOnLimit } = this.props;
		if (todos === undefined) {
			todos = [];
		}
		...
		const TableRow = ({ line }) => {
			const {regId, regType, deliveryNo, msisdn, simNo, simType, ratePlan, lineStatus, deviceStatus, donorAccountNo, donorType, addOnVal,contractInfoDataCR} = line;
			return (
	...
	const DeviceFulfillmentDrop = () => {
	 ...
	 return (		 
		...
		const DeviceFulfillmentTableRow = ({ line }) => {
			const inputName = 'topupByDealer' + line.msisdn 
			return (
				<React.Fragment>	
				{((this.props.user.isTradePartner === true||this.props.user.isTradePartner === 'true') && (!JSON.stringify(line.lineDeviceInfo).includes('Device Contract'))&&
				(line.lineDeviceInfo !== null && line.lineDeviceInfo !== undefined && line.lineDeviceInfo.phoneModel !== undefined)) &&(
					...
					<Form size='tiny'>
						<Grid style={{ paddingLeft: 25 }}>
							...
							<Grid.Row style={{ paddingTop: 20 }}>
								...
								<Grid.Column width='4' style={{ paddingBottm: 0 }} floated='right'>Total Member:<span style={{paddingLeft:5}}>{todos.length-crpLineCount+lineCount-activeKenanContractCount}</span></Grid.Column>
							</Grid.Row>
							<Grid.Row style={{ padding: 0 }}>
...
const mapStateToProps = (state) => {
	return {
		...
		pkgList:state.configuration.data.pkgList,
