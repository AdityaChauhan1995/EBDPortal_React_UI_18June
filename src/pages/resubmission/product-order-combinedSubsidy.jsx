...
import { setProductOrderInfoERF, setLineDetails, getFundAmount,getMsisdnDetails,setProductOrderInfoPrevERF, setRTFDeviceCount,validateStock } from '../../redux/actions/order';
...
class ProductOrder extends Component {
	constructor(props) {
		...
		console.log('deviceContracts', props.deviceContracts);
		this.state = {
			...
			validateOldLinesStatus:props.validateOldLinesStatus,
			count:0,
			getmsisdnsDetailsStatus:props.getmsisdnsDetailsStatus,
			oldLinesMsisdn:[],
			msisdnInLineValidation:'',
	...
	componentDidMount() {	
		let { todos,getmsisdnsDetailsStatus } = this.state;
        let { orderCategory, selectedVSN,msisdnList,easMasterRegId } = this.props;
			console.log(this.props.deviceContracts);
			if (todos === undefined || todos === null) {
				this.setState({ todos: [] });
			}
			
			if(getmsisdnsDetailsStatus ===true && this.state.validateOldLinesStatus === "PENDING")
			{
				let todos=[];	
				let tempZerolutionRTFfDeviceCount = 0;
				let tempNonRTFDeviceCount = 0;
				let color = '';
				this.state.msisdnList.map((msisdnInfo,key) => {
				let tempLineDeviceInfo='';
				if(msisdnInfo.lineDeviceInfo !== null && msisdnInfo.lineDeviceInfo !== undefined && msisdnInfo.lineDeviceInfo !== '')
				{
				if(msisdnInfo.lineDeviceInfo.deviceId===null || msisdnInfo.lineDeviceInfo.phoneModel===null)
				{
					tempLineDeviceInfo=null;
				}
				else {
					tempLineDeviceInfo=msisdnInfo.lineDeviceInfo;
					if(msisdnInfo.deviceContract === 'Normal' || (msisdnInfo.deviceContract === 'Zerolution' &&  msisdnInfo.lineDeviceInfo !== undefined && msisdnInfo.lineDeviceInfo !== null
					 	&& msisdnInfo.lineDeviceInfo.phoneModel !== null && (msisdnInfo.lineDeviceInfo.imeiNo === null || msisdnInfo.lineDeviceInfo.imeiNo === undefined
					 	|| msisdnInfo.lineDeviceInfo.imeiNo === ''))){
						tempNonRTFDeviceCount++;
					}

					if(msisdnInfo.deviceContract === 'Zerolution' && msisdnInfo.lineDeviceInfo !== undefined && msisdnInfo.lineDeviceInfo !== null
					 	&& msisdnInfo.lineDeviceInfo.imeiNo !== null && msisdnInfo.lineDeviceInfo.imeiNo !== undefined
					 	&& msisdnInfo.lineDeviceInfo.imeiNo !== ''){
				...
				let vasList=[];	
				if(msisdnInfo.pendingActionContracts.componentPackageInfoList !== null && 
					msisdnInfo.pendingActionContracts.componentPackageInfoList !== undefined &&
				    msisdnInfo.pendingActionContracts.componentPackageInfoList.length !== 0){
				for(var i=0;i<msisdnInfo.pendingActionContracts.componentPackageInfoList.length;i++)
				{
                   let  vasList1={vasId:msisdnInfo.pendingActionContracts.componentPackageInfoList[i].vasId,
						componentDesc:msisdnInfo.pendingActionContracts.componentPackageInfoList[i].componentDesc,
						newComponentId:msisdnInfo.pendingActionContracts.componentPackageInfoList[i].componentId,
						newPackageId:msisdnInfo.pendingActionContracts.componentPackageInfoList[i].packageId		 
						}
						vasList.push(vasList1);
				}
			}
			if(msisdnInfo.activeKenanMsisdnFlag){
				color = '#F4AFAE';
			}else{
				color = '';
			}
			todos=[...todos,{
			regId: msisdnInfo.regId,
			deviceContract: msisdnInfo.deviceContract,
			regType: msisdnInfo.regType,
			regTypeId: msisdnInfo.regTypeId,
			deliveryNo: msisdnInfo.deliveryNumber,
			ratePlan: msisdnInfo.ratePlanName,
			ratePlanId: msisdnInfo.ratplan,
			promotion: msisdnInfo.promotion,
			promotionId: msisdnInfo.promotionId,
			msisdn: msisdnInfo.mobileInfo.mobileNo,
			simNo: msisdnInfo.mobileInfo.sim,
			simType: msisdnInfo.mobileInfo.simType,
			donorType: msisdnInfo.mobileInfo.donorType,
			donorAccountNo: msisdnInfo.mobileInfo.donorAccountNo,
			planType: msisdnInfo.mobileInfo.planType,
			lineDeviceInfo: tempLineDeviceInfo,
			lineStatus: msisdnInfo.lineStatus,
			deviceStatus: msisdnInfo.lineDeviceInfo.status,
			vasList:vasList,
			iddList: msisdnInfo.iddList,
			billable:msisdnInfo.billableInd,
			color:color,
			activeKenanMsisdnFlag:msisdnInfo.activeKenanMsisdnFlag
			} ]
			});
			this.setState({
				todos:todos,
				...
				{
			  	if(todos[i].msisdn !== null && todos[i].msisdn !== undefined && todos[i].msisdn !== '')
			  	{
				oldLinesMsisdn.push(todos[i].msisdn);
				  }
				}
				this.setState({oldLinesMsisdn:oldLinesMsisdn,
							   validateOldLinesStatus:"ONGOING",
							   showDimmer:true
								});
				this.validatingOldMsisdns(todos);	
			}		
	}

	addLineOldMsisdns(currentLine){ 
			...
			let tempUpgradeSafeDevice = 'No', tempSafeDevice = 'No';
				let {msisdn,simNo,regType,deviceContract,ratplan,lineDeviceInfo,donorAccountNo,promotionId,ratePlanId}=currentLine;
				
				if(lineDeviceInfo !== undefined &&  lineDeviceInfo !== null && lineDeviceInfo !== '' )
				{
					deviceId = lineDeviceInfo.deviceId;
				}
				if(lineDeviceInfo !== undefined &&  lineDeviceInfo !== null && lineDeviceInfo !== '' )
				{
				if(currentLine.lineDeviceInfo.isPremiumDeviceProtection === 'Yes')
				{
					tempSafeDevice= 'Yes';
				}
			}
				if(currentLine.vasList.length>0 && currentLine.vasList.length !== null && currentLine.vasList.length !== undefined){
					let strVasIdsTemp ='';
				for(var i=0;i<currentLine.vasList.length;i++)
				{
					strVasIdsTemp += currentLine.vasList[i].vasId;
						if(i !== currentLine.vasList.length-1)
						{
							strVasIdsTemp = strVasIdsTemp + ',';
						}
				}
				strVasIds=strVasIdsTemp;
			}
			else{
                strVasIds='Test';
			}
			console.log("str vasids",strVasIds,simNo);
				if (deviceContract === 'Normal') {
					if (deviceId === undefined || deviceId === null || deviceId === '' || deviceId === null)
					 {
						 deviceId = 0; }
				} else {
					promotionId = -1
				...
				let tempSimNo = simNo;
				if (regType !== 'Normal Registration' && regType !== 'Number Porting' && regType !== 'MISC' && regType !== 'New Registration') {
					tempSimNo = 'Test';
				}
				if (regType !== 'Number Porting') {
					donorAccountNo = 'Test';
				}
				if (regType === 'Phone Only Order' || regType === 'Contract Renewal' || msisdn.includes('Outright_')) {
					ratePlanId = -1;
					strVasIds = 'Test'
				...
				if(regType==='CRP')
				{
                    regTypeNew='Change Rate Plan';
				}
				else if (regType==='AddVas')
				{
                    regTypeNew='Add VAS';
				}
				else {
				...
				this.setState({msisdnInLineValidation:msisdn});
				this.props.validateLineERF(msisdn, regTypeNew, deviceContract,tempSimNo, salesChannelId, brn, tempSafeDevice,
									tempUpgradeSafeDevice, ratePlanId, deviceId,strVasIds, 'No', donorAccountNo, promotionId, 0, userRole,null);
				
						}

		
    validatingOldMsisdns(todos) {
		let {count} = this.state;
		let regTypeNew='';
		if(count<todos.length){
		if(todos !== undefined && todos !== null && todos.length !== 0)
		{
			if(todos[count].regType==='CRP')
			{
				regTypeNew='Change Rate Plan';
			}
			else if (todos[count].regType==='AddVas')
			{
				regTypeNew='Add VAS';
			}
			else {
				regTypeNew=todos[count].regType;
			}	
		if((todos[count].regType === "CRP" || todos[count].regType === 'AddVas' ||
			todos[count].regType === 'SI Transfer' ||todos[count].regType === "Contract Renewal"||
			todos[count].regType === 'Phone Only Order') && (!todos[count].msisdn.includes('Outright_')))
					 {
				console.log("contractCheckERF called for");
				this.setState({showDimmer:true,msisdnInLineValidation:todos[count].msisdn});
				this.props.contractCheckERF( todos[count].msisdn,regTypeNew, todos[count].deviceContract, this.props.brn);
					}
			else {
				console.log("addLineOldMsisdns called");
			 	this.addLineOldMsisdns(todos[count]);
			//  this.setState({showDimmer:true});	
			}
		}
	}
	else {
		this.setState({validateOldLinesStatus:"SUCCESS"});
	...
	componentWillReceiveProps(nextProps) {
       if (this.props.FETCH_REG_TYPES_STATUS !== 'SUCCESS' &&
			...
			console.log('nextProps.msisdnContracts', nextProps.msisdnContracts);
			if(this.state.validateOldLinesStatus === 'ONGOING' ){
				this.setState({contractDurationGrtrFourMonths:nextProps.msisdnContracts.contractDurationGrtrFourMonths});
				this.addLineOldMsisdns(this.state.todos[this.state.count]);
			}
			else{
			this.setState({
			...
			if(this.state.deviceContract === 'Zerolution' && this.state.regType === 'Phone Only Order'){
				...
				console.log(nextProps.msisdnContracts.msisdn)
				if (nextProps.msisdnContracts.errorMsg !== null && nextProps.msisdnContracts.errorMsg !== '') {
				if(this.state.validateOldLinesStatus === 'ONGOING')
				{
					...
					message: nextProps.msisdnContracts.errorMsg+" for Msisdn "+nextProps.msisdnContracts.msisdn,
					validateOldLinesStatus:'FAILURE',
					  showDimmer:false});
				}
				else {
		...
		if(!(this.state.validateOldLinesStatus === 'ONGOING'))
		{
			...
			this.setState({contractDurationGrtrFourMonths:false});
		}
		...
		if(this.state.deviceContract === 'Zerolution' && this.state.regType === 'Phone Only Order'){
			...
			nextProps.VALIDATE_LINE_STATUS === 'SUCCESS') {
				if((this.state.validateOldLinesStatus === 'ONGOING') ){
					let {todos,count,contractDurationGrtrFourMonths}=this.state;
					if(contractDurationGrtrFourMonths)
					{
					let tempTodoValue = {...todos[count],contractDurationGrtrFourMonths:true};
					let tempTodos = todos;
					tempTodos[count]=tempTodoValue;
					this.setState({todos:tempTodos,contractDurationGrtrFourMonths:false});
					}
					
					if(this.state.count === this.state.oldLinesMsisdn.length - 1){
						this.setState({validateOldLinesStatus:'SUCCESS',showDimmer:false},() =>{
							if(this.state.nextButtonClicked)
							{
								console.log("next url called")
								this.next();
							}
						});
					}
			...
			let { todos, deviceContract, regType, regTypeId, ratePlan, ratePlanId, promotion, promotionId, vasList, iddList, mobileNumber, simCardNo,
				...
				quantity,contractDurationGrtrFourMonths, isZerolutionRTF, zerolutionRTFfDeviceCount, imeiNo, nonRTFDeviceCount} = this.state;
			...
			if (iddList === '') { iddList = []; }
			if (vasList === '') { vasList = []; }
			if (regType === 'Contract Renewal' || regType === 'Phone Only Order' ) { ratePlanId = 0 }
			...
			nextProps.VALIDATE_LINE_STATUS === 'FAILED') {
				console.log("validatation failedddd")
				if((this.state.validateOldLinesStatus === 'ONGOING')){
					this.setState({
						status: 'FAILURE', message: nextProps.validateErrorMessage+" for Msisdn "+this.state.msisdnInLineValidation,
						 showDimmer: false,isContractCheckDone: false,
						 validateOldLinesStatus:"FAILURE",
						 contractDurationGrtrFourMonths:false
					});
				}
		...
		else if(this.props.SET_PRODUCT_ORDER_PREVIOUS_ERF_STATUS !== 'SUCCESS' &&
		nextProps.SET_PRODUCT_ORDER_PREVIOUS_ERF_STATUS === 'SUCCESS'){
		this.props.history.goBack();
		}

		else if(this.props.SET_LINE_DETAILS_STATUS !== 'SUCCESS' &&
		nextProps.SET_LINE_DETAILS_STATUS === 'SUCCESS'){
			console.log("dimmer to false")
			this.setState({ showDimmer: false });
		}
		...
		nextProps.GET_BCC_VALIDATION_STATUS === 'SUCCESS')
		{
			...
			console.log("bccvalidation",bccValidationFailed);
		...
		let { mobileNumber, regType,deviceContract,simCardNo,promotionId, ratePlanId,safeDevice,deviceId,upgradeSafeDevice,strVasIds,donorAccountNo,vasIdsSelected  } = this.state;
		...
		this.props.validateLineERF(mobileNumber, regType, deviceContract, tempSimCardNo, this.props.user.salesChannelId, this.props.brn, tempSafeDevice,
			tempUpgradeSafeDevice, ratePlanId, deviceId, strVasIds, 'No', donorAccountNo, promotionId, 0, this.props.user.userRole,vasIdsSelected);
	}

	}
        
	next = () => {
		let { todos, status, lineCount,validateOldLinesStatus} = this.state;
		let { brn } = this.props;
		let  isContractDurationGrtrFourMonths =false;
		this.setState({nextButtonClicked:true});
		if(validateOldLinesStatus !== 'SUCCESS')
		{
			this.setState({ count:0,validateOldLinesStatus:"ONGOING",oldLinesMsisdn:todos,
		                    showDimmer:true},() =>{
		        this.setState({status: 'SUCCESS'});
				this.validatingOldMsisdns(this.state.todos);
			});			
       
		}
		if(validateOldLinesStatus !== "FAILURE"){
		...
		for (var i = 0; i < todos.length; i++) {
			console.log("reg types ",todos[i].regType);
			if(todos[i].regType === 'Normal Registration' || todos[i].regType === 'SI Transfer' || todos[i].regType === 'Number Porting' || 
				...
				todos[i].regType === 'New Registration' || todos[i].regType === 'Port-in'
				//||  todos[i].regType === 'Contract Renewal'
					...
					if(todos[i].regType === 'Number Porting' || todos[i].regType === 'Port-in'){
						if(!todos[i].activeKenanMsisdnFlag){
		...
		let onlyMnpOrder = true;
		let allActiveMNPLines = true;
		for (var i = 0; i < todos.length; i++) {
			if((todos[i].regType === 'Normal Registration' || todos[i].regType === 'SI Transfer' || todos[i].regType === 'Number Porting' ||
				todos[i].regType === 'New Registration' || todos[i].regType === 'Port-in' ) && !todos[i].activeKenanMsisdnFlag )
				{
			...
			if((todos[i].regType === 'Number Porting' || todos[i].regType === 'Port-in')&&
					!todos[i].activeKenanMsisdnFlag){
				allActiveMNPLines = false;
			}
				
		}

		if(onlyMnpOrder && allActiveMNPLines){
			this.setState({ status: 'FAILURE', message: 'Order should have atleast 1 valid MNP line. ', showDimmer: false });
			return;
		}
		...
		if (status === 'SUCCESS') {
			todos.map((currentLine) => {
				...
				msisdnList = [...msisdnList,
				{
					...
					activeKenanMsisdnFlag:currentLine.activeKenanMsisdnFlag
				}
				]
			})
		...
		console.log("new msisdns",msisdnList);
	}
	}
	previous = () => {
		let { nonRTFDeviceCount, zerolutionRTFfDeviceCount, isZerolutionRTF } = this.state;

		   console.log(this.state.validateOldLinesStatus)
		   this.props.setRTFDeviceCount( nonRTFDeviceCount ,zerolutionRTFfDeviceCount, isZerolutionRTF);
			this.props.setProductOrderInfoPrevERF(
				this.state.validateOldLinesStatus,this.state.todos);
	...
	addLineOutright = () => {
		...
		if(!isRegTypeDeviceIdSame){
			...
			this.props.validateLineERF(mobileNumber, 'regType', deviceContract, 'tempSimCardNo', salesChannelId, brn, 'No',
				'No', 20, deviceIdOutright, 'strVasIds', 'Yes', 'donorAccountNo', 20, quantity, userRole,null);
		}
		else{
			let tempTodos =todos;
	...
	handleChange = (e, { name, value }) => {
		...
		if (name === 'regType') {
			...
			if (value === 'Phone Only Order' || value === 'Contract Renewal') {
		...
		if(name === 'simCardNo' || name === 'imeiNo'){
			...
			let { promotionId, deviceContract, mobileNumber } = this.state;
	...
	vasRuleCheck(ratePlanId, strVasIds) {
		...
		this.setState({ showDimmer: true, strVasIds: strVasIds,vasIdsSelected:strVasIds });
	...
	render() {
		...
		//console.log('msisdnContracts', msisdnContracts, msisdnContracts !== null, msisdnContracts.length);
		...
		let {groupName,masterRegId,brnInfo,custBrnNo,masterRegStatus,prodCatName}=this.props.registrationDetails;
		if (todos === undefined || todos === null) {
			todos = [];
        }
		...
		const TableRow = ({ line }) => {
			...
			return (
				<Grid.Row  style={{ background:line.color, display: 'flex', flexDirection: 'row', alignItems: 'center', borderBottom: '0.5px solid rgba(78, 78, 78, 0.4)', padding: 2 }}>
					<Grid.Column width={2} style={{ paddingRight: 0 }}>
						<div>{(line.deviceContract) === "Normal"?'Maxis':(line.deviceContract)}</div>
						<div>{regId}</div>
						{
                         (line.msisdn.includes('Outright'))?
		...
		return (
			<Container fluid>
				...
				<Segment basic style={{ padding: 0, paddingTop: 15, flex: 1 }}>
					<Form size='tiny'>
					<Grid style={{ paddingLeft: 10, paddingBottom: 0 }}>
						 <StaticBlock8
							 custBrnNo={custBrnNo}
...
const mapStateToProps = (state) => {
	return {
		...
		getmsisdnsDetailsStatus:state.order.data.getmsisdnsDetailsStatus,
		validateOldLinesStatus:state.order.data.validateOldLinesStatus,
		SET_PRODUCT_ORDER_PREVIOUS_ERF_STATUS:state.order.meta.SET_PRODUCT_ORDER_PREVIOUS_ERF_STATUS,
