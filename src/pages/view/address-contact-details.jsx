...
class AddressContactDetails extends Component {
	constructor(props) {
        ...
        if(this.props.groupName === 'MAXIS' || this.props.groupName === 'Zerolution'){
            tempNextUrl = url.substring(0, url.lastIndexOf("/")) + "/product-order-combinedSubsidy?mode=View"
		}else if(this.props.groupName === 'Business Postpaid with Fibre Option'){
		
			tempNextUrl = url.substring(0, url.lastIndexOf("/")) + "/product-order-obs?mode=View"
		}
		else{
            tempNextUrl = url.substring(0, url.lastIndexOf("/")) + "/product-order?mode=View"
        }
		this.state = {
			nextUrl: tempNextUrl,
			validateResult: {},
	...
	next = () => {
		let tempNextURL=this.state.nextUrl;
			tempNextURL+="&easMasterRegId="+this.props.easMasterRegId;
				...
				<Segment basic style={{ padding: 0, margin: 0, flex: 1 }}>
					<Form size='small' autoComplete='off'>
						<Grid style={{paddingLeft: 10,paddingTop: 30}}>
							 ...
							 {groupName==='MAXIS' &&
					 			<StaticBlock8 custBrnNo={custBrnNo} companyName={portalCustInfo.companyName} masterRegId={masterRegId}
