...
class VasActionMaxis extends Component {
    ...
    show(vasList, msisdnNo, ratePlanName, existingVasList, existingRebateList, type, regType, prematurePenalty) {
        if (type === 'VIEW') {
            this.setState({
                mobileNo: msisdnNo,
    ...
    // save = () => {
    //     let {existingVasList, prematurePenalty} = this.state;
    ...
    //
    render() {
        const {open, dimmer, vasList, ratePlanName, mobileNo, existingVasList, existingRebateList, type, regType, disabled, prematurePenalty} = this.state
        let {status, message} = this.state.validationResult;
        console.log(existingVasList);
        return (
            <Modal dimmer={dimmer}
                   open={open}
                   onClose={this.close}
                ...
                <Modal.Header style={{fontSize: 13}}>
                        <label style={{color: '#4E4E4E'}}>Contracts - View</label>
                </Modal.Header>
                <Modal.Content style={{minHeight: 50}}>
                    <Grid style={{height: 400, overflow: 'auto'}}>
                        ...
                        {(existingVasList !== null && regType === 'Contract Renewal') && (<React.Fragment>
                            <Grid.Row>
                                <Grid.Column width='8'>
                        ...
                        {(existingVasList !== null && regType !== 'Contract Renewal') && (
                        <React.Fragment>
                            <Grid.Row>
                                <Grid.Column width='8'>
