...
class AddressContactDetails extends Component {
	constructor(props) {
		...
		if(this.props.bundleType === 'MAXIS'){
			tempNextUrl = url.substring(0, url.lastIndexOf("/")) + "/product-order-combinedSubsidy"
		}else if(this.props.bundleType === 'Business Postpaid'){
		tempNextUrl = url.substring(0, url.lastIndexOf("/")) + "/product-order"
		}else{
            tempNextUrl = url.substring(0, url.lastIndexOf("/")) + "/product-order-obs"
		}
	...
	handleChange = (group, name, value) => {
		...
		switch (group) {
			...
			//console.log('hi',name,name!=='contactPhoneNo',name!=='contactFixNo',(name==='contactPhoneNo' || name==='contactFixNo'),checkNum);
			if(name !== 'contactPhoneNo' && name !== 'contactFixNo')
			{ 
				this.setState({
			...
			var checkValNumeric = false;
				...
				break;
				case "SecondaryPIC":
			      var checkValNumerical = false;
				if(name==='secondaryPicContactNumber'||name==='secondaryPicIc'||name==='secondaryPicContactFixedNo')
				{
					checkValNumerical = this.checkIsNumeric(value);

					if(checkValNumerical){
					if(value.length<8){
				   ...
				   this.setState({
					...
					}else if(name === 'secondaryPicContactNumber'||name === 'secondaryPicIc'||name === 'secondaryPicContactFixedNo' && checkValNumerical){
						this.setState({
							secondaryPicDetails: { ...this.state.secondaryPicDetails, [name]: value },
	...
	render() {
		let { billingAddress, deliveryAddress, picDetails, useBillingAddress, validateResult, showDimmer,secondaryPicDetails } = this.state;
		let { brn, orderCategory, selectedVSN, allPostCodes, companyName, bundleType} = this.props;
		return (
			<Container fluid>
				<PleaseWait active={showDimmer} />
				<Navigation index={3} group={bundleType} />
				<Segment basic style={{ padding: 0, margin: 0 }}>
					<Form size='small'>
						<Grid style={{ paddingLeft: 25, paddingTop: 20, paddingBottom: 0 }}>
							{bundleType !='MAXIS' &&
								<StaticBlock1 brn={brn} orderCategory={orderCategory} selectedVSN={selectedVSN} companyName={companyName} />
							}
							{bundleType==='MAXIS' &&
								<StaticBlock7 brn={brn} orderCategory={orderCategory} masterRegId={this.state.masterRegId} companyName={companyName} />
							}
						</Grid>
...
const mapStateToProps = (state) => {
	return {
		...
		billingAddress: state.order.data.addressInfo.billingAddress,
		deliveryAddress: state.order.data.addressInfo.deliveryAddress,
		picDetails: state.order.data.addressInfo.picDetails,
		secondaryPicDetails: state.order.data.addressInfo.secondaryPicDetails,
