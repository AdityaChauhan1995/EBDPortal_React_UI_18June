...
const addressDetails = ({ mode,data, validateResult, postCodes, allPostCodes, handleChange, type}) => {
    return (
        <Grid style={{ paddingBottom:0,paddingTop:2,paddingLeft: 10 }} >
            <Grid.Row style={{ paddingBottom: 0, paddingTop: 0 }} columns={4}>
                <Grid.Column width={2} style={{ paddingTop: 8 }}>
                    ...
                    {(validateResult.status === 'FAILURE' && ((type === 'DELIVERY' && validateResult.missingdeliveryaddress1) || (type === 'BILLING' && validateResult.missingAddress1) ) ) && <Icon style={{ paddingLeft: 5 }} name='question circle' color='red' />}
                </Grid.Column>
                <Grid.Column width={6}>
            ...
            <Grid.Row style={{ paddingBottom: 0, paddingTop: 2 }} columns={4}>
                <Grid.Column width={2} style={{ paddingTop: 8 }}>
                        ...
                        {(validateResult.status === 'FAILURE' && ((type === 'DELIVERY' && validateResult.missingdeliveryaddress2) ||  (type === 'BILLING' && validateResult.missingAddress2) )) && <Icon style={{ paddingLeft: 5 }} name='question circle' color='red' />}
                </Grid.Column>
                <Grid.Column width={6}>
            ...
            <Grid.Row style={{ paddingBottom: 0, paddingTop: 2, display: 'flex', flexDirection: 'row', alignItems: 'center' }} columns={4}>
                <Grid.Column width={2} style={{ paddingTop: 8 }}>
                    <label>Postcode Hint</label>
                        {(validateResult.status === 'FAILURE' && ((type === 'DELIVERY' && validateResult.missingPostCodeDeliveryValue) ||  (type === 'BILLING' && validateResult.missingPostCodeValue) )) && <Icon style={{ paddingLeft: 5 }} name='question circle' color='red' />}
                </Grid.Column>
                <Grid.Column width={5}>
