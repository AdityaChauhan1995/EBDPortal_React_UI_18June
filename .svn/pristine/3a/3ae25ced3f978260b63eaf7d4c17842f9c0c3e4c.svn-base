...
import { getRegTypesobs, getRateplansobs, getRateplansMisc, getSimTypes, getDonorTypes, getZerolutionDevices, getOBSDeviceInfo, getVasPopupData, validateLine, fetchDeviceFundContracts, performVasRuleCheck, contractCheck, checkingBccValidation,getOBSDevices, getExistingShareableLineCountExcludeCRPMsisdns } from '../../redux/actions/configuration';
import { setProductOrderInfoOBS, setLineDetails, getFundAmount, validateOldMsidns, setProductOrderInfoPrev, getRatePlanData, setRatePlanBlank, saveInsAddress, setRTFDeviceCount,validateStock } from '../../redux/actions/order';
...
const PlanSelection = ({ regTypes, ratePlans, deviceContracts, deviceContract, regType, ratePlan, handleChange, openModal, orderCategory, addOnVal, addOnLimit, onClickIncrease, onClickDecrease,handleDeviceClick }) => {
	return (
		<Grid.Row >
...
const DeviceInfo = ({ regType, simCardNo, simTypes, simType, mobileNumber, deviceContract, zerolutionDevices, phoneModel, handleChange, handleChangeCheckBox, contractCheck, fetchRatePlanDetails,obsDevices, isZerolutionRTF, imeiNo, orderCategory, ratePlans, ratePlan  }) => {
	return (
		<React.Fragment>
...
class ProductOrder extends Component {
	constructor(props) {
		...
		this.state = {
			...
			installationAddress: props.installationAddress != null ? props.installationAddress : null,
			instMessage: '',
	...
	componentDidMount() {
		...
		let { todos, msisdnList } = this.state;
		...
		let crpMsisdnList = [];
		if(todos===undefined||todos == null){
			if (msisdnList !== null && msisdnList !== undefined && msisdnList.length > 0) {
				let countMisc = 0;
				var tempObsDevicesCount = 0;
				let contractCount=0;
				for (let i = 0; i < msisdnList.length; i++) {
					if (msisdnList[i].regType != 'Contract Renewal') {
						crpMsisdnList = [...crpMsisdnList, msisdnList[i].mobileInfo.mobileNo];
					}
					if (msisdnList[i].regType.includes('MISC') || msisdnList[i].regType.includes('VoiceGo')) {
						countMisc += 1;
					}
					if (msisdnList[i].regType === 'Contract Renewal') {
						contractCount += 1;
					...
					if(msisdnList[i].lineDeviceInfo !== undefined && msisdnList[i].lineDeviceInfo !== null
					 	&& msisdnList[i].lineDeviceInfo.phoneModel !== null && msisdnList[i].deviceContract==='1L1P Contract'){
						tempObsDevicesCount++;
					}

					if(msisdnList[i].lineDeviceInfo !== undefined && msisdnList[i].lineDeviceInfo !== null
					 	&& msisdnList[i].lineDeviceInfo.imeiNo !== null && msisdnList[i].lineDeviceInfo.imeiNo !== undefined
					 	&& msisdnList[i].lineDeviceInfo.imeiNo !== '' && msisdnList[i].deviceContract==='Zerolution'){
						if(this.props.isZerolutionRTF){
					...
					if(msisdnList[i].regType !== 'Contract Renewal'){
						if (msisdnList[i].ratePlanPkgId === 41987 || msisdnList[i].ratePlanPkgId === 41942 || msisdnList[i].ratePlanPkgId === 41943 ) {
							eligibleCountForSharebleLines += 1;
						}else if(msisdnList[i].ratePlanPkgId === 41944){
							eligibleCountForSharebleLines += 3;
						}else if(msisdnList[i].ratePlanPkgId === 41945){
							eligibleCountForSharebleLines += 5;
						}else if(msisdnList[i].ratePlanPkgId === 41946){
							eligibleCountForSharebleLines += 7;
						}else if(msisdnList[i].ratePlanPkgId === 41968){
				...
				this.setState({ countMisc: countMisc,obsDevicesCount:tempObsDevicesCount,phoneModel:'',
					lineDeviceInfo:'', obsRTFDeviceCount: tempObsRTFDeviceCount,obsNonRTFDeviceCount:tempNonRTFDeviceCount, 
					shareMemberAllowed: eligibleCountForSharebleLines, sharedMemberSubscribed: total48PlanShareableLinesTaken,
					activeKenanContractCount:contractCount });
			}
		}
		else
		{
		if (todos !== null && todos !== undefined && todos.length > 0) {
			...
			for (let i = 0; i < todos.length; i++) {
				if(todos[i].regType !== 'Contract Renewal'){
					crpMsisdnList = [...crpMsisdnList, todos[i].msisdn];
				}
				...
				if(todos[i].lineDeviceInfo !== undefined && todos[i].lineDeviceInfo !== null
					&& todos[i].lineDeviceInfo.phoneModel !== null ){
					tempObsDevicesCount++;
					tempNonRTFDeviceCount++;
				}
				if(todos[i].lineDeviceInfo !== undefined && todos[i].lineDeviceInfo !== null
				 	&& todos[i].lineDeviceInfo.imeiNo !== null && todos[i].lineDeviceInfo.imeiNo !== undefined
				 	&& todos[i].lineDeviceInfo.imeiNo !== ''&& todos[i].deviceContract==='Zerolution'){
					if(this.props.isZerolutionRTF){
						tempObsRTFDeviceCount++;
					}
			...
			this.setState({ countMisc: countMisc,obsDevicesCount:tempObsDevicesCount,phoneModel:'',lineDeviceInfo:'',
			 obsRTFDeviceCount: tempObsRTFDeviceCount,obsNonRTFDeviceCount:tempNonRTFDeviceCount, shareMemberAllowed: eligibleCountForSharebleLines, sharedMemberSubscribed: total48PlanShareableLinesTaken });
		}
	}
		let tempTotalDeviceTopUp = 0;
		for (let i = 0; i < msisdnList.length; i++) {
			if(msisdnList[i].lineDeviceInfo !== null && msisdnList[i].lineDeviceInfo.deviceTopUp !== undefined
				&& msisdnList[i].lineDeviceInfo.deviceTopUp !== null && msisdnList[i].lineDeviceInfo.deviceTopUp !== '' ){
					tempTotalDeviceTopUp += Number(msisdnList[i].lineDeviceInfo.deviceTopUp);
			}
		}
		...
		if (this.state.validateOldMsisdn === 'PENDING' && this.state.initialAddition === false) {
			...
			this.setState({
				...
				todos: this.props.msisdnList.map(currentLine => {
					...
					// if (currentLine.ratePlanName.indexOf("Contract") !== -1) {
					// 	tempFlexiFundContractTaken = true;
					// }
					// if (!(currentLine.ratePlanName.indexOf("Plan Only") !== -1 || currentLine.ratePlanName.indexOf("BYOD") !== -1)) {
					// 	tempFlexiFundContractTaken = true;
					// }
					if (currentLine.ratePlanName!==null && currentLine.ratePlanName.indexOf("Fund Contract") !== -1) {
						tempFlexiFundContractTaken = true;
					}
					...
					return {
						...
						ratePlanPkgId:currentLine.ratePlanPkgId
				...
				if (this.state.todos.length > 0) {
					...
					for (var i = 0; i < this.state.todos.length; i++) {
						if (this.state.todos[i].regType === 'MISC eSMS') {
							count++;
						} else {
							break;
						}
					...
					if (count === 0) {
						this.setState({ validateOldMsisdnCount: count });
					} else {
						this.setState({ validateOldMsisdnCount: count - 1 });
					}
					this.validatingOldMsisdn(this.state.todos[count]);
	...
	componentWillReceiveProps(nextProps) {
		if (this.props.FETCH_REG_TYPES_OBS_STATUS !== 'SUCCESS' &&
			...
			let tempInstMessage = {
				status: 'FAILURE',
				message: 'some error occured while saving installation address'
			}
			...
			let {  regType, mobileNumber, ratePlanId,simCardNo,vasIdsSelected  } = this.state;
			...
			let { todos, regType, mobileNumber, ratePlan, currentPage, simCardNo, simType, donorAccountNo, lineDeviceInfo,
				...
				addOnVal,deviceId,deviceTopUp,phoneModel, imeiNo, isZerolutionRTF,deviceContract,isActiveKenanContractPresent, ratePlanPkgId } = this.state;
				...
				if(phoneModel !== undefined && phoneModel !== null && phoneModel !== ''){
					...
					templineDeviceInfo = { ...this.state.lineDeviceInfo,
				...
				if(deviceContract==='Zerolution'){
					...
					this.setState({obsDevicesCount:deviceCount});
				}
				}
			if (iddList === '') {
				iddList = [];
				...
				for (let i = 0; i < temptodos.length; i++) {
					...
					if(temptodos[i].lineDeviceInfo !== null && temptodos[i].lineDeviceInfo.deviceTopUp !== undefined
						&& temptodos[i].lineDeviceInfo.deviceTopUp !== null && temptodos[i].lineDeviceInfo.deviceTopUp !== '' ){
							tempTotalDeviceTopUp += Number(temptodos[i].lineDeviceInfo.deviceTopUp);
					}
					if(temptodos[i].regType !== 'Contract Renewal'){
			...
			if (this.state.validateOldMsisdn === 'ONGOING') {
				...
				this.setState({
					...
					if (this.state.pendingOldMsisdnList.length > this.state.validateOldMsisdnCount) {
						var count = 0;
						for (var i = this.state.validateOldMsisdnCount; i < this.state.pendingOldMsisdnList.length; i++) {
							if (this.state.pendingOldMsisdnList[i].regType === 'MISC eSMS') {
								count++;
							} else {
								break;
							}
						}
						if (this.state.validateOldMsisdnCount + count === this.state.pendingOldMsisdnList.length) {
							...
							this.validatingOldMsisdn(this.state.pendingOldMsisdnList[this.state.validateOldMsisdnCount + count]);
							this.setState({ validateOldMsisdnCount: this.state.validateOldMsisdnCount + count })
						}
			...
			if (this.state.validateOldMsisdn === 'ONGOING') {
				...
				this.setState({
					status: 'FAILURE', validateOldMsisdn: 'FAILURE',
			...
			if (nextProps.msisdnList != []) {
				...
				if (this.props.orderCategory === 'Existing Group- Add VAS') {
					...
					this.setState({ showDimmer: true });
					console.log('deivceContractInfo',deivceContractInfo);
		...
		nextProps.GET_BCC_VALIDATION_STATUS === 'SUCCESS')
		{
			...
			if(bccValidationFailed)
			{
				...
				this.props.setProductOrderInfoOBS(this.state.regType,
					...
					this.state.activeKenanContractCount
			)
			}
		}
	...
	validatingOldMsisdn(list) {
		let { todos, regType, mobileNumber, ratePlan,
			...
			flexiFundContractTaken, ratePlanId, mobileNoValidSubscriber, isContractCheckDone, byodContractTaken,isZerolutionRTF,ratePlanPkgId } = this.state;
		...
		regType = list.regType,
			...
			ratePlanPkgId = list.ratePlanPkgId
		...
		if (ratePlan === undefined || ratePlan !== undefined && ratePlan !== null && ratePlan.trim() === '') {
			this.setState({ status: 'FAILURE', message: 'Please select rateplan first', showDimmer: false });
			return;
		}
		if ((regType !== undefined && regType.trim() !== 'Upgrade' && regType !== null && regType.trim() !== 'Downgrade') && mobileNumber.trim() === '') {
			this.setState({ status: 'FAILURE', message: 'Msisdn is mandatory', isMsisdnMissing: true, showDimmer: false })
			return;
		}
		else if ((regType !== undefined && regType !== null && (regType.trim() === 'MNP Port In' || regType.trim() === 'Normal Registration')) && (simCardNo !== undefined && simCardNo !== null && simCardNo.trim() === '')) {
			this.setState({ status: 'FAILURE', message: 'SIM Serial Number is mandatory', isSimNoissing: true, showDimmer: false })
			return;
		}
		else if ((regType !== undefined && regType !== null && regType.trim() === 'MNP Port In' || regType.trim() === 'Normal Registration') &&
			(simType !== undefined && simType !== null && simType.trim() === '')) {
			this.setState({ status: 'FAILURE', message: 'SIM Type is mandatory', isSimTypeMissing: true, showDimmer: false })
			return;
		}
			...
			this.setState({ status: 'FAILURE', message: 'Donor Type is mandatory', isDonorTypeMissing: true, showDimmer: false })
			return;
		}
			...
			this.setState({ status: 'FAILURE', message: 'Donor Account No is mandatory', isDonorAccountNoMissing: true, showDimmer: false })
			return;
		}
		if (deviceContract === 'Zerolution'  && isZerolutionRTF && (flexiFundContractTaken || byodContractTaken)) {
			this.setState({ status: 'FAILURE', message: 'Cannot take zerolution device with Fund Contract/ BYOD Contract', deviceContractCheckFail: true, showDimmer: false })
			return;
		}
		...
		if (mobileNumber.trim() !== '') {
			if (regType.trim() !== 'MISC FIBRE' && regType.trim() !== 'VoiceGo' && mobileNumber.length < 11 ) {
				this.setState({ status: 'FAILURE', message: 'Invalid Mobile Number. Length must be of 11 digit atleast.', isMsisdnMissing: true })
				return;
			}
			else if (regType.trim() === 'MISC FIBRE' && mobileNumber.length < 10 ) {
			...
			else if (regType.trim() !== 'VoiceGo' && mobileNumber.slice(0, 2) !== '60') {
				this.setState({ status: 'FAILURE', message: `Invalid Mobile Number ${mobileNumber}.  Please enter a valid number that starts with 60`, isMsisdnMissing: true, showDimmer: false })
				return;
			}
		}
		if (simCardNo !== null && simCardNo.trim() !== '') {
			if (simCardNo.length < 19) {
				this.setState({ status: 'FAILURE', message: `Invalid SIM Card number ${simCardNo}. Length is less than 19.`, isSimNoissing: true, showDimmer: false })
				return;
			}
		}
		...
		if (mobileNoValidSubscriber || (regType != null || regType != '')) {
			...
			if ((this.props.orderCategory==='Existing Group' && regType ==='Contract Renewal' && ratePlanId===null) ) {
				console.log('***validating contract renewal msisdn*****');
				this.props.validateLine(mobileNumber, tempSimCardNo, regType, tempVSN, this.props.user.userRole, this.props.user.salesChannelId, 'fd', 0,null,ratePlanPkgId, this.props.orderCategory);
			}else{
				console.log('***validating contract renewal msisdn ',mobileNumber,ratePlanId,this.state.ratePlanPkgId);
				this.props.validateLine(mobileNumber, tempSimCardNo, regType, tempVSN, this.props.user.userRole, this.props.user.salesChannelId, 'fd', ratePlanId,null,ratePlanPkgId, this.props.orderCategory);
			}
		}
		else {
			this.setState({ status: 'FAILURE', message: 'Not a valid subscriber', showDimmer: false });
			return;
		}
	...
	next = () => {
		...
		if (orderCategory === 'Existing Group') {
			...
			if (((todos.length + lineCount - tempCrpLineCount - countMisc- countContractRenewal) > 150)) {
		...
		for (var i = 0; i < todos.length; i++) {
			if ((orderCategory === 'New Group') && (todos[i].regType === 'Normal Registration' || todos[i].regType === 'Non Member Transfer' || todos[i].regType === 'MNP Port In') ||
				(orderCategory === 'Existing Group') && (todos[i].regType === 'Normal Registration' || todos[i].regType === 'Non Member Transfer' || todos[i].regType === 'MNP Port In' || todos[i].regType === 'Contract Renewal'))
		...
		let bccRequestData = {
			...
			groupName:'Business Postpaid with Fibre Option',
		...
		if (status === 'SUCCESS' && tempValidateoldMsisdns) {
			...
			/*this.props.setProductOrderInfoOBS(this.state.regType,
				...
				this.state.countMisc,
				obsRTFDeviceCount,
				isZerolutionRTF:tempobsRTFDeviceCount = (obsRTFDeviceCount !== null && obsRTFDeviceCount > 0)?true:false
			)*/
			//this.props.checkingBccValidation(brn,totalRequestingLine,strVasIds);
		}
	}
	previous = () => {
		let { todos, crpLineCount, lineCount,countMisc, obsRTFDeviceCount, isZerolutionRTF,obsNonRTFDeviceCount,activeKenanContractCount } = this.state;
		let { orderCategory } = this.props;
		if (orderCategory === 'Existing Group- Add VAS') {
			this.props.setRatePlanBlank('');
		}
		this.props.setRTFDeviceCount(obsNonRTFDeviceCount , obsRTFDeviceCount, isZerolutionRTF);
		this.props.setProductOrderInfoPrev(todos.length - crpLineCount-countMisc + lineCount - activeKenanContractCount);
	...
	addLine = () => {
		let { todos, regType, mobileNumber, ratePlan,
			simCardNo, simType, donorAccountNo, donorType, deviceContract,
			flexiFundContractTaken, ratePlanId, mobileNoValidSubscriber, isContractCheckDone, byodContractTaken, crpLineCount, isaddMemberCheckValid, installationAddress,totalDeviceTopUp,countMisc,
			isZerolutionRTF, safeDevice, obsRTFDeviceCount, deviceId, imeiNo, obsDevicesCount,obsNonRTFDeviceCount , isaddLineValid,isValidateStock,phoneModel,vasIdsSelected, ratePlanPkgId,isVasRUleCheckValid } = this.state;
		/*if(!isaddLineValid){
			var msg = this.state.message;
			this.setState({ status: 'FAILURE', message: msg });
			return;
		}*/
		...
		if(deviceContract === '1L1P Contract' && isZerolutionRTF){
			this.setState({ status: 'FAILURE', message: 'Please uncheck Real Time Fulfillment if you want to take 1L1P Contract' });
			return;
		}
		...
		if(deviceContract === 'Zerolution' && isZerolutionRTF && deviceId !== '' && obsRTFDeviceCount>5){
		...
		if(deviceContract === 'Zerolution' && isZerolutionRTF && (ratePlan.includes('Contract') || ratePlan.includes('contract')||ratePlan.includes('24')||ratePlan.includes('Mths')) && deviceId !== ''){
				this.setState({ status: 'FAILURE', message: 'Cannot take zerolution device with this Contract' });
				return;
		}
		...
		if (regType !== 'Group Level Add-On' && regType.trim() !== 'MISC eSMS' && mobileNumber.trim() !== '') {
			if (regType !== 'MISC FIBRE' && regType !== 'VoiceGo' && mobileNumber.length < 11 ) {
				this.setState({ status: 'FAILURE', message: 'Invalid Mobile Number. Length must be of 11 digit atleast.', isMsisdnMissing: true })
				return;
			}
			else if (regType === 'MISC FIBRE' && mobileNumber.length < 10 ) {
				this.setState({ status: 'FAILURE', message: 'Invalid Mobile Number. Length must be of 10 digit atleast for MISC FIBRE.', isMsisdnMissing: true })
				return;
			}
		...
		// if(this.props.orderCategory!==null && this.props.orderCategory!==undefined && this.props.orderCategory!=='Existing Group- Add VAS' && !regType.includes('MISC')){
		...
		if (!(regType.includes('MISC') || regType.includes('VoiceGo') || regType ==='Contract Renewal') && (todos.length - countMiscs - countContractRenewal >= 150)) {
			this.setState({ status: 'FAILURE', message: 'Limit Reached, You can not add more than 150 lines.' });
			return;
		}
		let tempTotalDeviceTopUp = 0;
		for (let i = 0; i < todos.length; i++) {
			if(todos[i].lineDeviceInfo !== null && todos[i].lineDeviceInfo.deviceTopUp !== undefined
				&& todos[i].lineDeviceInfo.deviceTopUp !== null && todos[i].lineDeviceInfo.deviceTopUp !== '' ){
					tempTotalDeviceTopUp += Number(todos[i].lineDeviceInfo.deviceTopUp);
			}
		}
		this.setState({totalDeviceTopUp:tempTotalDeviceTopUp});
		...
		if (mobileNoValidSubscriber) {
			...
			if(vasIdsSelected==undefined || vasIdsSelected=='')
				vasIdsSelected=null;
			if (!(regType.includes('MISC') || regType.includes('VoiceGo') || regType =='Contract Renewal' )) {
				this.props.validateLine(mobileNumber, tempSimCardNo, regType, tempVSN, this.props.user.userRole, this.props.user.salesChannelId, 'fd', ratePlanId,vasIdsSelected,  this.state.ratePlanPkgId, this.props.orderCategory);
			} else {
				this.props.validateLine(mobileNumber, 'tempSimCardNo', regType, tempVSN, this.props.user.userRole, this.props.user.salesChannelId, 'fd', ratePlanId,vasIdsSelected,  this.state.ratePlanPkgId, this.props.orderCategory);
			}
	...
	removeRow(row) {
		let { todos, currentPage, todosPerPage, pendingOldMsisdnList, validateOldMsisdn, crpLineCount, obsRTFDeviceCount, isZerolutionRTF } = this.state;
		let tempToDos = null;
		let tempCrpLineCount = 0;
		let countMisc = 0;
		...
		tempToDos.map((x) => {
			...
			if(x.lineDeviceInfo !== undefined && x.lineDeviceInfo !== null &&  x.deviceContract ==='1L1P Contract'){
				tempObsDevicesCount++;
			}
			if (x.deviceContract==='Zerolution' && x.lineDeviceInfo !== null ) {
				if(x.lineDeviceInfo.imeiNo !== undefined && x.lineDeviceInfo.imeiNo !== null && x.lineDeviceInfo.imeiNo !== ''){
					tempObsRTFDeviceCount++;
				}else{
		...
		this.setState({
			...
			isaddLineValid: tempToDos.length==0?true:false,
		...
		this.props.setLineDetails(tempToDos, tempCrpLineCount, countMisc);
	}
	...
	handleChange = (e, { name, value }) => {
		...
		let { safeDevice, isZerolutionRTF,deviceContract } = this.state;
		if ((name === 'mobileNumber' || name === 'simCardNo' || name === 'donorAccountNo' || name === 'imeiNo')) {
			...
			if (name === 'ratePlan') {
				...
				this.setState({phoneModel:'',lineDeviceInfo:'',deviceId:'',deviceTopUp:'',deviceContract:''});
			}
			if(name === 'deviceContract'){
				...
				if(value === ''){
					this.setState({ phoneModel: '', lineDeviceInfo: '', deviceId: '', deviceTopUp: null, deviceContract: '',rpMasterId:-1,isZerolutionRTF:false,status:'SUCCESS',message:'',isaddLineValid:true });
				}else{
						this.setState({isZerolutionRTF:this.state.isZerolutionRTF,status:'SUCCESS',message:'',isaddLineValid:true});
				}
			}
			this.setState({ [name]: value });
		}
		...
		if (name === 'regType') {
			if (value !== 'Group Level Add-On' && value !== 'Member Level Add-VAS' &&
				...
				value !== 'MISC eSMS') {
					...
					lineDeviceInfo:'',deviceId:'',deviceTopUp:'' });
			...
			// 		}
			// 	})
			// 	this.setState({ regTypeId: regTypeId })
			// 	this.props.getRateplansMisc(value, 'test')
			// }
			...
			this.props.ratePlans.map((rateplanRow) => {
				if (rateplanRow.value === value) {
					...
					this.setState({ showDimmer: true });
					if (this.state.regType !== undefined && this.state.regType !== null && this.state.regType === 'Member Transfer') {
	...
	handleChangeCheckBox = (e, { name, checked }) => {
		let {obsDevicesCount, obsRTFDeviceCount, obsNonRTFDeviceCount, deviceContract}=this.state;
		if (name === 'zerolutionSafeDevice') {
			this.setState({ [name]: checked, zerolutionSafeDevice: checked });
		}else if (name === 'isBizAddress') {
			this.setState({ [name]: true, show: checked })
			this.newBizAddress.current.show();
		}else if(name === 'zerolutionRTF'){
			this.setState({ [name]: checked, isZerolutionRTF: checked
			//	, regType: '', ratePlan: '', deviceContract:'' 
			});
			if(checked){
				if(obsNonRTFDeviceCount>0 || obsDevicesCount-obsNonRTFDeviceCount>0 &&obsRTFDeviceCount===0){
					this.setState({ status: 'FAILURE', message: 'Delete all Zerolution and 1L1P Contract lines before adding Real Time Fulfillment lines', isaddLineValid:false });
					return;
				}else{
					this.setState({ status: '', message: '', isaddLineValid:true });
				}
				...
				// 		]
				// })
				if(obsRTFDeviceCount>0 ){
					this.setState({ status: 'FAILURE', message: 'Delete all Real Time Fulfillment lines before adding Non Real Time Fulfillment lines',isaddLineValid:false });
					return;
				}else{
					this.setState({ status: '', message: '',isaddLineValid:true });
				}
			...
			if(this.state.deviceId!==''){
					this.props.getOBSDeviceInfo(this.state.deviceId, '000', checked, deviceContract==='Zerolution');
	...
	vasRuleCheck(ratePlanId, strVasIds) {
		...
		this.props.performVasRuleCheck(ratePlanId, strVasIds, 'Business Postpaid with Fibre');
	}
	onDismiss() {
	...
	close(proceed) {
		if (proceed === 'yes') {
			...
			this.props.setProductOrderInfoOBS(null, null, null, null, null, null, null, null, null, null, this.state.todos, [], null, null, null, 0)
		}
		else {
	...
	render() {
		let {
			todos, currentPage, todosPerPage, regType, ratePlan, mobileNumber, simCardNo,
			simType, phoneModel, deviceContracts, deviceContract, indexOfLastTodo, indexOfFirstTodo,
			currentTodos, donorType, firstIndexCurrentPage, donorAccountNo, status,
			message, showDimmer, vasRule, isValidVAS, msisdnContracts,
			lineCount, crpLineCount, addOnVal, allPostCodes,
			isBizAddress, instMessage, countMisc,obsDevices,totalDeviceTopUp, obsDevicesCount, isZerolutionRTF, imeiNo,
			obsNonRTFDeviceCount,obsRTFDeviceCount,activeKenanContractCount, shareMemberAllowed, sharedMemberSubscribed
		    } = this.state;
		...
		const TableHeader = () => {
			...
			return (
				<Grid.Row style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', borderBottom: '2px solid rgba(78, 78, 78, 0.2)', padding: 2 }}>
					{(orderCategory !== 'Existing Group- Add VAS') && (
						<React.Fragment>
		...
		const TableRow = ({ line }) => {
			let { isZerolutionRTF } = this.state;
			const { regId, regType, deliveryNo, msisdn, simNo, simType, ratePlan, lineStatus, deviceStatus, donorAccountNo, donorType, addOnVal, deviceContract } = line;
			return (
				<Grid.Row style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', borderBottom: '0.5px solid rgba(78, 78, 78, 0.4)', padding: 2 }}>
					...
					{(orderCategory !== 'Existing Group- Add VAS' && !regType.includes('MISC') && !regType.includes('VoiceGo')) && (
						<React.Fragment>
							...
							<Grid.Column width={2} style={{ paddingRight: 0 }}>
								{
									(ratePlan != undefined && ratePlan != null && ratePlan != 0) && (
										<div>{ratePlan}</div>
									)
								}
							</Grid.Column>
									...
									<Grid.Column width={2} style={{ paddingRight: 0 }}>
										{
											(line.lineDeviceInfo !== null && line.lineDeviceInfo !== undefined && line.lineDeviceInfo.phoneModel !== undefined && line.lineDeviceInfo.phoneModel !== null) && (
												<React.Fragment>
													...
													{
														(this.state.isZerolutionRTF == true) && (
															<React.Fragment>
							...
							{	( deviceContract==='Zerolution' && !isZerolutionRTF ) && (
									<Grid.Column width={1} style={{ paddingRight: 0 }}>
					...
					{(regType.includes('MISC') || regType.includes('VoiceGo')) && (
						<React.Fragment>
							...
							<Grid.Column width={1} style={{ paddingRight: 0, paddingLeft: 15 }}>
								{regType === 'MISC FIBRE' &&
									<Checkbox onChange={this.handleChangeCheckBox} name='isBizAddress' checked={isBizAddress} value={this.state.isBizAddress} />
								}
							</Grid.Column>
							<Grid.Column width={1} style={{ paddingRight: 0, paddingLeft: 15 }}>
		...
		return (
			<Container fluid>
				...
				<Segment basic style={{ padding: 0, paddingTop: 15, flex: 1 }}>
					<Form size='tiny'>
						<Grid style={{ paddingLeft: 10 }}>
							<StaticBlock5 custBrnNo={custBrnNo} prodCatName={prodCatName} companyName={brnInfo.portalCustInfo.companyName} masterRegId={masterRegId}
								virtualServiceNo={virtualServiceNo} easPackageName={easPackageName} totalMembers={totalMembers} totalMemberString={todos.length - crpLineCount - countMisc + lineCount -activeKenanContractCount}
						...
						<Grid style={{ paddingLeft: 25 }}>
							...
							<Grid.Row style={{ paddingTop: 0 }}>
								<Grid.Column width='4' style={{ paddingLeft: 0, paddingBottm: 0 }} >
									<label style={{ color: '#293895' }} className='heading'>Business Postpaid with Fibre</label>
								</Grid.Column>
							</Grid.Row>
							<Grid.Row style={{ padding: 0 }}>
							...
							{ (	this.props.user.isEligibleForZerolutioRTF && deviceContract==='Zerolution' ) &&
								(
									<Grid.Row style={{ padding: 0, paddingTop:10 }}>
										<Grid.Column width='16' style={{ paddingLeft: 0, paddingBottm: 0 }}>
											<Checkbox style={{ padding: 5, fontWeight: 'bold', fontSize: 15, paddingLeft:0 }}  name='zerolutionRTF'  label='Real Time Fulfilment' checked = {this.state.isZerolutionRTF} onClick={this.handleChangeCheckBox} />
										</Grid.Column>
									</Grid.Row>
								)
							}
					...
					<Grid style={{ fontSize: 11, padding: 10, paddingLeft: 25 }}>
						...
						{ todos !== undefined && todos !== null && todos.length >0 &&
							<StaticBlock11 totalLines={obsNonRTFDeviceCount+obsRTFDeviceCount+obsDevicesCount} totalDeviceTopUp={totalDeviceTopUp}/>
						}
					</Grid>
					<Confirm
						cancelButton='NO'
				...
				<NewBizAddress ref={this.newBizAddress} allPostCodes={allPostCodes} installationAddress={this.state.installationAddress} onSaveInsAddress={(installationAddress) => this.onSaveInsAddress(installationAddress)} instMessage={instMessage} onDismiss={() => this.onDismiss()} />
			</Container>
		)
	}
}
const mapStateToProps = (state) => {
	return {
		...
		obszerolutionDeviceCount: state.order.data.obszerolutionDeviceCount,
